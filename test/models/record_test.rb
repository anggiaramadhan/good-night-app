...
class RecordTest < ActiveSupport::TestCase
  ...
  test 'should create record when clock_out greater than or equal clock_in' do
    record = Record.new(user_id: user.id, clock_in: Time.now, clock_out: Time.now + 8.hours, complete: true)
  ...
  test 'should not create record when clock_out less than clock_in' do
    record = Record.new(user_id: user.id, clock_in: Time.now, clock_out: Time.now - 8.hours)
  ...
  test 'should not create record when clock_out provide but complete is false' do
    record = Record.new(user_id: user.id, clock_in: Time.now, clock_out: Time.now + 8.hours, complete: false)
